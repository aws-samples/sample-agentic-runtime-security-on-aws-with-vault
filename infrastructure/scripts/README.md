...
|--------|---------|---------|-------------|
| `check-prerequisites.sh` | Workshop user, `workshop-e2e.sh` | Single entry-point: installs CLI tools (kubectl 1.34.x, helm 3.12+, terraform 1.10+, vault 1.21.x, aws v2, jq, yq), then verifies Bedrock model access, AWS service quotas (EC2 vCPU + EIP + RDS + AOSS OCU indexing/search), IAM permissions for the workshop bootstrap, and Vault Enterprise prerequisites (`hashicorp/vault` provider >= 5.10.1, Enterprise license file present at `VAULT_ENTERPRISE_LICENSE_PATH`). Mirrors `eks-terraform-stacks/infrastructure/scripts/check-prerequisites.sh` (same name, broader workshop-specific checks) | PREF-01, PREF-02, PREF-03, PREF-05 |
...
| `uc3-virtual-authenticator.py` | `verify-uc3.sh --no-phone` (admin only) | Headless MMFA device that lets an admin prove the Use Case 3 CIBA chain end-to-end with no IBM Verify app installed. Enrols over the documented endpoints the app itself uses (OAuth `authorization_code` for the `AuthenticatorClient`, scope `mmfaAuthn`, then SCIM for the response key) and answers the real user-presence challenge with an RSA signature — no gate is stubbed or relaxed. Runs inside the `uc3-agent` pod. Refuses to touch a device it did not enrol, and deletes its own on the way out. Not part of the attendee path: attendees tap Approve on a real phone | — |
...
| `deploy-workshop.sh` | Workshop user | Tier-by-tier deploy orchestrator (`--tier 1\|2\|3`). Flags: `--image-source <ghcr\|ecr>` (default `ecr`), `--ghcr-registry-base <base>` (e.g. `ghcr.io/<githubusername>` — required in `ghcr` mode, no default), `--skip-infra`, `--skip-build`, `--skip-vault-init`, `--skip-acme`. In `ghcr` mode: no build, no ECR, no deployment roll. In `ecr` mode: builds via `build-*.sh`, pushes to ECR, rolls deployments to pull `:latest`. Tier-2 preflight resolves the three tier-2 secrets into the gitignored `infrastructure/services/terraform.tfvars`: the two IBM secrets (`icr_entitlement_key`, `ivia_mmfa_push_client_secret`) come from the `ICR_ENTITLEMENT_KEY` / `IVIA_MMFA_PUSH_CLIENT_SECRET` env vars when set, otherwise a hidden prompt; the Vault Enterprise license is read from a file (`VAULT_ENTERPRISE_LICENSE_PATH`, default `~/Downloads/vault-ent.hclic`) — read fresh and overwritten on every run. Precedence per secret: existing tfvars value → env var → prompt → fail-fast (non-interactive). | — |
...
| `publish-images.sh` | Maintainer only (NOT in the attendee path) | Builds the workshop images (`--platform linux/amd64`) and pushes them to your own public GHCR base (bring your own — no default; set via `GHCR_REGISTRY_BASE` env or `--registry-base ghcr.io/<githubusername>`, required). Publishes all five at `:v1` by default. `--image <name>` (repeatable: `uc1-agent`, `banking-ui`, `banking-agent`, `banking-mcp`, `uc3-agent`) restricts to a subset; `--version <tag>` (default `v1`, **requires `--image`**) bumps only that image — so unchanged images never get a meaningless new tag (e.g. `--image banking-ui --version v2`). Inlines its own build (does NOT call `build-*.sh`). Requires a `write:packages` PAT via `GHCR_PAT` env or `gh auth token`. Idempotent, `--dry-run`. | — |
...
`ghcr_registry_base` has no default — it is a bring-your-own value, required when `image_source = "ghcr"`. No ECR repos provisioned, no `aws ecr` call, no container runtime required, no deployment roll.

`ghcr` mode is bring-your-own: publish the five workshop images to your own GHCR first (`publish-images.sh --registry-base ghcr.io/<githubusername>`), then pass `--ghcr-registry-base ghcr.io/<githubusername>` — this cascades `ghcr_registry_base` into the tier-3 Terraform apply. The publish base (`publish-images.sh --registry-base`) and the consume base (`deploy-workshop.sh --ghcr-registry-base`) must match; pointing consume at an empty or private base yields `ImagePullBackOff`. If you select `ghcr` mode without a base, the deploy fails fast before any AWS work.
...
# Publish to your own namespace (required — no default)
