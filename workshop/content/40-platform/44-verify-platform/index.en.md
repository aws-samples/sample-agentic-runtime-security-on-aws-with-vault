...
**IVIA pods in ImagePullBackOff** — The `ibm_entitlement_key` HCP Terraform variable is missing or incorrect. Update the variable set and re-run the `verify_access` component apply.
...
**cert-manager or AWS LBC not running** — These are deployed in the `addons` component. Re-run the addons apply from HCP Terraform if they are missing.
