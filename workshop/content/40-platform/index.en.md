...
2. [Deploy Verify Access](./42-deploy-verify-access/) — provision IBM IVIA 11.0.2, PostgreSQL schema, and ALB Ingress.
3. [Configure the OIDC Seam](./43-configure-oidc-seam/) — wire Vault `kubernetes` and `jwt` auth methods, dynamic Postgres/AWS secrets engines, and IVIA OAuth clients.
...
```
  Attendee browser ──► IVIA ALB ──► IVIA OIDC provider
                                          │
                                    issues JWT (sub=user, act=agent)
                                          │
...
The Vault `jwt` auth method is configured with IVIA's OIDC discovery URL as the `oidc_discovery_url`. Every JWT Vault receives from an agent is verified against IVIA's JWKS endpoint, making IVIA the authoritative identity plane for all user-delegated operations.
...
| `verify_access` (IVIA + ALB) | 6–10 minutes |
| `vault_config` + `isva_config` | 2–3 minutes |
| **Total** | **~15 minutes** |
