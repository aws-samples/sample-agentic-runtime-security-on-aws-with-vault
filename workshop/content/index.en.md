...
**Pre-flight required** — Before deploying any infrastructure, complete the Prerequisites module: obtain IVIA licenses, then run `infrastructure/scripts/check-prerequisites.sh` (consolidated install + Bedrock + quotas + IAM checks).
...
| 30 | [Deploy Foundation](30-deploy-foundation/) | Deploy the full stack (VPC, EKS, RDS, Bedrock KB, Vault, IBM Verify Identity Access) via local Terraform, then verify each layer |
