...
| **Use Case 3** — three-plane audit correlation (IVIA approval + RDS pgaudit write joined by `request_id`; the Vault authentication is bound into the same row by credential path & time-proximity) | **End-to-end tracing and attribution** — audit events carry the agent-registry identity; correlation reads "agent X acting for human Y" |
