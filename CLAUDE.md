...
- **Every script MUST be idempotent and safe to re-run end-to-end — no exceptions.** This is non-negotiable and applies especially to `deploy-workshop.sh` and the whole deploy flow: a second `bash deploy-workshop.sh` (or any deploy/build/seed script) must succeed without errors, manual cleanup, or "already exists" failures, skipping or re-converging already-done work. New steps you add inherit this rule — design re-runnability in from the start, never as an afterthought. Provide `--skip-*` flags for slow re-runnable steps where it helps.
